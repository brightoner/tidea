...
<script type="text/javascript">
	...
	function fn_gridCompoSetting(){
		...
		fn_gridColgroupSetting('grid', ['10%', '10%', '10%', '*%', '10%', '10%', '10%','10%', '10%']); // colgroup 설정
		fn_gridAlignSetting('grid', ['center', 'center', 'center', 'left', 'center', 'center', 'center', 'center', 'center']); // 그리드 데이터 정렬 설정(종류:left, center, right)
	}
	...
	// 조회 버튼
	function fn_search(){
		var url = "/apply/applyMng.do?APPLY_NO=" +  $('#APPLY_NO').val() + "&APLCT_NO=" + $('#APLCT_NO').val() + "&INVT_NM=" + $('#INVT_NM').val() + "&STATUS=" + $('#STATUS').val();
		goSearch(url);
	}
	
	// 페이징 조회
	function pageCall(pageNo) {
		var url = "/apply/applyMng.do?";
		goPageCall(url, pageNo);
	}
	
	// 신규등록
	function fn_registration(){
		var url = "/apply/applyReg.do";
		goSearch(url);
	...
	<form name="form" method="post" accept-charset="UTF-8">
		...
		<div class="box-blue-line clear relative">
			...
			<div id="buttonDivBox" class="button_box">
				...
				<button type="button" id="regBtn" onclick="javascript:fn_registration();return false;"><span></span>등록</button><!-- 조회버튼 -->
			</div>
			<!-- 상단버튼 영역 END -->
