...
// 		fn_compoInputboxReadOnly('M', 'I', 'detail_1_3', 'CODE_DC', 'CODE_DC', '', '코드구분 설명');
			...
			success : function(result) {
				...
				if(d.length > 0){
					for(var i = 0; i < d.length; i++){
						...
						var val5 = d[i].ADDRESS == undefined ? '' : d[i].ADDRESS;
						var val6 = d[i].SAUPJA_NO == undefined ? '' : d[i].SAUPJA_NO;
						var val7 = d[i].USE_AT;
						var checked
						...
						str += '<tr id="tr'+i+'">';
							...
							str += '<td><input type="text" name="ADDRESS" value="'+val5+'" /></td>';
							str += '<td><input type="text" name="SAUPJA_NO" value="'+val6+'" /></td>';
							str += '<td>';
	...
	function fn_add(){
		...
		str += '<tr id="tr'+trLen+'">';
			...
			str += '<td><input type="text" name="ADDRESS" value="" /></td>';
			str += '<td><input type="text" name="SAUPJA_NO" value="" /></td>';
			str += '<td>';
				str += '<select name="USE_AT">';
					str += '<option value="Y">사용</option>';
					str += '<option value="N">사용안함</option>';
	...
	function fn_save(){
		...
		if(result){
			...
			var saupjaNoArr = new Array();
			var useAtArr = new Array();
			
			$('#detailTbody').find('tr').each(function(idx){
				var $td = $(this).children();
				codeSeArr[idx] = $td.eq(1).text();
				codeNmArr[idx] = $td.eq(2).text();
				...
				addressArr[idx] = $td.eq(5).children().val() == '' ? ' ' : $td.eq(5).children().val(); //ADDRESS
				saupjaNoArr[idx] = $td.eq(6).children().val() == '' ? ' ' : $td.eq(6).children().val(); //SAUPJA_NO
				useAtArr[idx] = $td.eq(7).children().val() == '' ? ' ' : $td.eq(7).children().val(); //USE_AT
			});
			...
			var paramData = {'codeSe':CODE_SE,'codeSeArr':codeSeArr, 'codeNmArr':codeNmArr, 'dtlcodeArr':dtlcodeArr, 'dtlcodeNmArr':dtlcodeNmArr, 'addressArr':addressArr, 'saupjaNoArr':saupjaNoArr, 'useAtArr':useAtArr};
						
			$.ajax({
				url : '/auth/savePrOrgDtlCdMng.do',
